...
class Batch(Base):
    ...
    harvest_id = Column(String, ForeignKey("harvests.id", ondelete="CASCADE"), unique=False, nullable=True)
    farmer_id = Column(String, ForeignKey("farmer_profiles.id", ondelete="CASCADE"), nullable=True)
    farm_id = Column(String, ForeignKey("farms.id", ondelete="CASCADE"), nullable=True)
    product_name = Column(String, nullable=False)
